...
- `gist run` — deterministic Playwright capture + pixelmatch diff → writes `.gist/`
- `gist ui` — local HTTP server serving a SPA that reads `.gist/`, never calls an LLM
- `/gist` skill — Claude Code skill that reads `evidence.json` + screenshots → writes `summary.md`
...
- `evidence.json` is deterministic — same inputs, same output. `summary.md` is the only AI-authored file.
